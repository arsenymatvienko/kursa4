	...
	while (true)
	{
		...
		case 5:
		{
			...
			case 6:
			{
				...
				int* end = pArr6 + size6 * size6;
				for (int* i = pArr6; i < end; i++)
				{
					*i = rand() % size6 * size6 + 1;
				}
				showMatrix(pArr6, size6);
				cout << endl;
				for (int* j = pArr6; j < end; j++)
				{
					*j = rand() % size6 * size6 + 1;
				}
				showMatrix(pArr6, size6);
				cout << "Сложенные матрицы:\n";
				int* end6 = pArr6 + size6;
				for (int* i = pArr6; i < end6; i++)
				{
					cout << setw(4) << *i;
